...
jQuery(document).ready(function() { 
 ...
 //only accept numbers and aplhabet
  jQuery('.miglaNumAZ').on('keydown', function (e){
    ...
    jQuery('.migla_number_only').on('keyup', function (e){
		...
		if( isNaN(mg_current_value) )
		{
			if(e.keyChar == '-' && mg_current_array_value[0] == '-' )
			{
				mg_new_val = mg_current_value ;
			}else{
				for( var i = 0; i < ( mg_current_array_value.length - 1 ) ; i = i + 1 )
				{			
					mg_new_val = mg_new_val + mg_current_array_value[i]; 			
				}				
			}	
